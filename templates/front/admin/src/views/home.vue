...
<div class="content" :style='{"padding":"30px"}'>
	...
	<div :style='{"margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('rongyuzhengshu','首页总数')">
			...
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{rongyuzhengshuCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>荣誉证书总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 2 -->
	<div class="type2" :style='{"alignContent":"flex-start","padding":"10px 20px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="rongyuzhengshuChart1" class="echarts1" v-if="isAuth('rongyuzhengshu','首页统计')"></div>
		<div id="rongyuzhengshuChart2" class="echarts2" v-if="isAuth('rongyuzhengshu','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getrongyuzhengshuCount();
		this.rongyuzhengshuChat1();
		this.rongyuzhengshuChat2();
	},
	methods:{
		// 统计图动画
		...
		getrongyuzhengshuCount() {
			this.$http({
				url: `rongyuzhengshu/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.rongyuzhengshuCount = data.data
				}
			})
		},
// 1234 饼
        ...
        var rongyuzhengshuChart1 = echarts.init(document.getElementById("rongyuzhengshuChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                rongyuzhengshuChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    rongyuzhengshuChart1.resize();
                };
            }
        });
      })
    ...
    rongyuzhengshuChat2() {
      this.$nextTick(()=>{
        ...
        var rongyuzhengshuChart2 = echarts.init(document.getElementById("rongyuzhengshuChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                rongyuzhengshuChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    rongyuzhengshuChart2.resize();
                };
            }
        });
      })
